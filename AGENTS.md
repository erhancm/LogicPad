...
- App-only extra: live-record of **host** keys, backup/restore, big-screen editor, **launch a PC program** from a pad key (drag a file onto the key; app must be running; mapping is local JSON, not flash). Typed strings share the packed flash slot with profiles (max 240 bytes per key) and are one step in that key’s macro (so Enter or a chord can follow). **Auto-switch** is a node graph in the companion app: Foreground / Running / AND / OR conditions wire into prioritized Set profile (with lights) or Restore previous. First matching priority wins. Window picker shows live thumbnails; Browse still picks an .exe. Rules live in `profile-rules.json` (graph + flattened exe list). Do not enumerate every window on the HID tick, and never wait forever on `GetWindowText` / `PrintWindow` (that froze the app after launch). Windows tray app; `SET_ACTIVE` is not saved to flash.
...
**Auto-switch** (Windows): while the tray app is running, a node graph in **Edit graph…** decides the pad profile. Condition nodes are **Foreground is** (focused exe), **Running is** (exe has a window), **AND**, and **OR**. Action nodes are **Set profile** (priority number, lights, per-key LEDs) and **Restore previous**. Lower priority number is evaluated first; the first true action wins. Unmatched programs restore the profile that was active before the graph took over. Pick programs from a thumbnail grid of open windows, or Browse to an `.exe`. Old `profile-rules.json` lists (`exe` + `profile`) load as a graph. YAML packs still export a flat exe list. Autostart enables when a launch mapping or an enabled auto-switch rule exists. `SET_ACTIVE` does not mark the store dirty (update firmware if Alt-Tab pops the OLED save prompt).
