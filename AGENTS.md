...
- **On-device config:** 0.96" SSD1306 128×64. Profiles, keys, macros, lights, screen. Home/sleep shows the active profile while you are using the PC; the 24-hour clock appears when USB is gone or Windows is locked / logged off (app `SET_HOST`).
...
- App-only extra: live-record of **host** keys, backup/restore, big-screen editor, **launch a PC program** from a pad key (drag a file onto the key; app must be running; mapping is local JSON, not flash). Typed strings share the packed flash slot with profiles (max 240 bytes per key) and are one step in that key’s macro (so Enter or a chord can follow). **Auto-switch** is a node graph in the companion app: Foreground is / Running is / AND / OR / Set profile / Set lights / Restore, wired on a grid canvas. Window picker shows live thumbnails; Browse still picks an .exe. Rules live in `profile-rules.json` (graph + flattened exe list). Do not enumerate every window on the HID tick, and never wait forever on `GetWindowText` / `PrintWindow` (that froze the app after launch). Windows tray app; `SET_ACTIVE` is not saved to flash.
- All nine pad keys are macros. Selector is **not** a 10th macro. Factory: four empty profiles (empty labels and actions). **+ New** adds profiles until the 4 KB slot is full. Empty keys take no flash. Magic `LPAI` `0x4C504149`; boot migrates `LPAH` (`0x4C504148`).
...
**Auto-switch** (Windows): while the tray app is running, the **Auto-switch** tab is a node graph. Drag yellow ports to wire Foreground is / Running is through AND/OR into Set profile, Set lights, or Restore previous profile. Empty-input Restore is ELSE and must not steal a matching Set profile. Pick programs from a thumbnail grid of open windows, or Browse to an `.exe`. Old `profile-rules.json` lists (`exe` + `profile`) and card-style graphs still load. YAML packs still export a flat exe list. Autostart enables when a launch mapping or an enabled auto-switch rule with at least one program exists. `SET_ACTIVE` does not mark the store dirty (update firmware if Alt-Tab pops the OLED save prompt).

Daily Windows use: installed **LogicPad** (`%LOCALAPPDATA%\LogicPad\LogicPad.exe`) — Start menu or desktop shortcut, no terminal. **The owner does not rebuild or run npm.** After app or HID/firmware-protocol changes, the agent must close `LogicPad.exe`, run `npm run build:app` in `04 Software/logicpad-app`, then silent-install `src-tauri/target/release/bundle/nsis/LogicPad_0.1.0_x64-setup.exe /S`, and start the installed app again. Do not leave the Start-menu copy stale. `npm run tauri dev` is only for live development while iterating.

**After app or HID/firmware-protocol changes, also reinstall that Start-menu copy** (close `LogicPad.exe`, `npm run build:app`, then `LogicPad_0.1.0_x64-setup.exe /S`). Do not leave the installed program stale. Do not ask the owner to run those commands.
