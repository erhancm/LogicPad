...
Connect with the pad plugged in. Edit profiles, the 3×3 keys, macros, and lights. **New profile** / **Delete** add or remove a profile (until the pad’s flash slot is full). **Save** writes flash on the device. **Update firmware** takes the app `LogicPad.bin` (not the factory hex).
...
**Auto-switch** is a visual node graph (**Auto-switch** tab). Wire **Foreground is** / **Running is** / **AND** / **OR** into a **Set profile** node (priority, lights, 3×3 LEDs) or **Restore previous**. Lower priority number wins. Pick programs from window thumbnails or Browse to an `.exe`. The tray app must stay running; Start with Windows turns on when a program is listed. Rules are local (`profile-rules.json`, graph plus a flattened exe list), not flash. Firmware that does not dirty on `SET_ACTIVE` avoids an OLED save prompt on every Alt-Tab. The background switcher only enumerates other windows when a **Running is** node exists, and it never waits forever on a hung window title.
