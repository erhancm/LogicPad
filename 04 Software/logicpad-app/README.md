...
**Auto-switch** is a visual node graph (Profiles aside → **Edit graph…**). Wire **Foreground is** / **Running is** / **AND** / **OR** into a **Set profile** node (priority, lights, 3×3 LEDs) or **Restore previous**. Lower priority number wins. Pick programs from window thumbnails or Browse to an `.exe`. The tray app must stay running; Start with Windows turns on when a program is listed. Rules are local (`profile-rules.json`, graph plus a flattened exe list), not flash. Firmware that does not dirty on `SET_ACTIVE` avoids an OLED save prompt on every Alt-Tab.
